...
def dim_vehicle_view():
    df = spark.readStream.table('silver_obt')
    df = df.select('vehicle_id', 'vehicle_make', 'vehicle_model', 'vehicle_type', 'vehicle_color', 'vehicle_make_id', 'vehicle_type_id', 'license_plate')
...
def dim_payment_view():
    df = spark.readStream.table('silver_obt')
    df = df.select('payment_method_id', 'payment_method', 'requires_auth', 'is_card')
